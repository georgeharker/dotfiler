...
local f l t
local -a files links
files=( README.md index.md(N) docs/*.md(N) example_install/README.md(N) )
for f in $files; do
...
if [[ -f "$map" ]] && git rev-parse main >/dev/null 2>&1; then
    ...
    check_coverage main docs/how-updates-work.md 3
    check_coverage main docs/authoring-install-files.md 2
fi
