...
#   Entry points (called in-process by update.zsh or by the setup.zsh wrapper):
...
#   Cleanup:
#     setup_unload        — undefine all functions and globals set by this lib
#
...
#
#   Full action dispatch. Reads action arrays set by setup_main arg parsing:
...
# setup_main "$@"
#
#   Full entrypoint: parse argv, validate, and dispatch.  setup.zsh sources
#   this lib and calls setup_main "$@" — nothing else.  All arg-parsing logic
#   lives here so the lib is self-contained and testable.
#
...
#   rather than going through setup_main.
function setup_main() {
...
# setup_unload
#
#   Undefine all functions and globals set by setup.zsh.
...
# If executed directly (not sourced), run setup_main with all args.
